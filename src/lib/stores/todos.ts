...
async function api(path: string, opts?: RequestInit) {
	...
	const signature = await signRequest(signingKey, method, path, body, timestamp);
...
export function setupSync() {
	...
	// Sign the SSE connection (EventSource doesn't support custom headers)
	const message = `GET\n/api/sync\n${timestamp}\n`;
	const encoded = new TextEncoder().encode(message);

	// We need to synchronously build the URL but signing is async,
	// so we start the EventSource after signing
	let es: EventSource | undefined;
	const signPromise = crypto.subtle.sign('Ed25519', signingKey, encoded).then(sigBuf => {
		let binary = '';
